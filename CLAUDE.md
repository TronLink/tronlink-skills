...
Instead of the CLI you can register the bundled MCP server, which exposes **39 `tron_*` tools** backed by the same data:

```bash
claude mcp add -s user tronlink-skills -- node "$HOME/.tronlink-skills/scripts/mcp_server.mjs"
```
